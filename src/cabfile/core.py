...
class ERF(Structure):
    _pack_ = _STRUCT_PACK
    _fields_ = [("erfOper", c_int), ("erfType", c_int), ("fError", BOOL)]

    def __repr__(self):
        return "<ERF: erfOper:%s erfType:%s fError:%s" % (
            self.erfOper,
            self.erfType,
            self.fError,
        )
...
def TCOMPfromLZXWindow(w):
    return ((w) << tcompSHIFT_LZX_WINDOW) | (tcompTYPE_LZX)
...
class FDIDECRYPT(Structure):
    ...
    _pack_ = _STRUCT_PACK
    _fields_ = [("fdidt", FDIDECRYPTTYPE), ("pvUser", py_object), ("u", _U)]
    _anonymous_ = ("u",)
...
def is_cabinetfile(filename):
    ...
    hfdi = FDICreate(
        a.malloc, a.free, f.open, f.read, f.write, f.close_cb, f.seek, 0, byref(e)
    )
    try:
...
class CabinetFile(object):
    ...
    def printdir(self):
        ...
        for cinfo in self.infolist():
            date = (
                cinfo.datetime.strftime("%Y-%m-%d %H:%M:%S")
                if cinfo.datetime
                else "unknown"
            )
